...
# =========================================================
# Optional GPU detection (for WSL + PyCharm interpreter)
# =========================================================
USE_GPU = True
...
def to_cpu_numpy(measurement):
    out = measurement.compute()
    if hasattr(out, "to_cpu"):
        out = out.to_cpu()
...
# ---------------------------------------------------------
# FIXED scan pixel count
# probe 조건이 바뀌어도 이 값 유지
# ---------------------------------------------------------
scan_nx = 32
scan_ny = 32

# True면 repeat_x x repeat_y supercell 중 중앙 1 tile만 스캔
...
defocus_values_A = np.arange(0.0, sample_thickness_A + 1e-9, defocus_step_A)
...
for df in probe_defocus_values_A:
    ...
    arr, _ = to_cpu_numpy(probe_vac.build().intensity())
    probe_images.append(arr)

probe_images = np.asarray(probe_images)
...
plt.close(fig_probe)
print("[Probe] Figure saved. Continuing to ADF simulation...")
...
if hasattr(probe_waves, "to_cpu"):
    probe_waves = probe_waves.to_cpu()
...
scan_width_x_A = (end_x - start_x) * atoms_sim.cell.lengths()[0]
scan_width_y_A = (end_y - start_y) * atoms_sim.cell.lengths()[1]
pixel_size_x_A = scan_width_x_A / scan_nx
pixel_size_y_A = scan_width_y_A / scan_ny

print(f"Scan gpts = ({scan_nx}, {scan_ny})")
print(f"Scan width = ({scan_width_x_A:.4f} Å, {scan_width_y_A:.4f} Å)")
print(f"Pixel size = ({pixel_size_x_A:.4f} Å/pix, {pixel_size_y_A:.4f} Å/pix)")
...
for df in defocus_values_A:
    ...
    adf_mean = adf_measurement.reduce_ensemble()
    arr, _ = to_cpu_numpy(adf_mean)
    adf_stack.append(arr)
